  ...
  return(
    <div className='Listcard'>
      ...
      <div className='Listcard2'>
        <h2>{VehicleName}</h2>
        <ul>
          <li><div><FaCar/> </div>Category: {Category}</li>
          <li><div><FaGears/> </div>Transmission: {Transmission} </li>
          <li><div><BsFillFuelPumpFill/> </div> Fuel Type: {FuelType}</li>
          <li><div><FaUsers/> </div> Seats: {Seats}</li>
          <li><div><PiFanFill/> </div>Air Conditioning: {AirConditioning}</li>
        </ul>
      </div>
...
function Listing() {
  return (
    <div className='ListingFather'>
      <div><h1 className='head'>Cars to be Booked</h1></div>
      <div>
        {list.map((current, i) =>(
          <Listcard
          key={i}
